...
def insertarSalesKafka(fecha_formateada, store_ID, store_name, product_ID, product_name, quantity_sold, revenue):
    try:
        connection = psycopg2.connect(host='localhost' , port='5432',database='warehouse_retail_db' , user= 'postgres', password='casa1234' )

        cursor = connection.cursor() 
        cursor.execute("INSERT INTO ventas (date, store_ID, store_name, product_ID, product_name, quantity_sold, revenue) VALUES (%s, %s, %s, %s, %s, %s, %s);", 
                       (fecha_formateada, store_ID, store_name, product_ID, product_name, quantity_sold, revenue))
...
for message in consumer:
    ...
    try:
        ...
        if diferencia_tiempo <= 1:                                  # Tiempo es menor a 1 segundo, procesar el mensaje
            ...
            #date, store_ID, store_name, product_ID, product_name, quantity_sold, revenue
            insertarSalesKafka(fecha_formateada, store_ID, store_name, product_ID, store_name, quantity_sold, revenue)
        else:
            print()
...
# Producer  python3
# & C:/Users/Adrian/AppData/Local/Microsoft/WindowsApps/python3.10.exe "./Tema 4/Spark/data/Salesdirect/kafka_producer.py"

# Consumer python
# & C:/Users/Adrian/AppData/Local/Microsoft/WindowsApps/python3.10.exe "./Tema 4/Spark/data/Salesdirect/kafka_consumer.py"
 
# & C:/Users/Adrian/AppData/Local/Microsoft/WindowsApps/python3.10.exe "./Tema 4/Spark/data/Salesdirect/kafka_producer.py"
# python 'c:\Users\Adrian\Desktop\BDA_Adrian\Tema 4\Spark\data\ESTRUCTURAExamen\data_Prim_ord\test\kafka_producer.py'